...
<td>ID:  <%=complaint.getId()%></td>
</tr>

<tr>

<td>
COMPLAINT TYPE<font color="red">*</font>:<br/>
...
<td>
COMPLAINT DT<font color="red">*</font>:<br/>
...
<td>
COMPLAINT STATUS<font color="red">*</font>:<br/>
...
<input type="hidden" name="page" value= "saveShowNextComplaint">
<input type="hidden" name="pageNo" value= "<%=pageNo%>">
<input type="hidden" name="limit" value= "<%=limit%>">
<input type="hidden" name="id" value= "<%=complaint.getId()%>">
</form>

<form action="complaintCntrl"  id="myform1" name="myform1" accept-charset="UTF-8" >
<input type="hidden" name="page" value= "saveShowNextComplaint">
<input type="hidden" name="pageNo" value= "<%=pageNo%>">
<input type="hidden" name="limit" value= "<%=limit%>">
<input type="hidden" name="id" value= "<%=complaint.getId()%>">
...
<% dropDownDBService.closeConnection(); %>
    ...
    <% Iterator<Complaint> it1= complaintList.iterator(); %>
    
<%if(pageNum!=0){ %>   	
	<div align="left">	
	<font color="blue" size="3">	
	<%if(pageNum==1){%>|&lt;&nbsp;&nbsp;&nbsp;&lt;&lt;<%} else { %><a href="complaintCntrl?page=complaintDashboard&opr=showAll&pageNo=<%=1%>&limit=<%=limit%>">|&lt;</a>&nbsp;&nbsp;&nbsp;<a href="complaintCntrl?page=complaintDashboard&opr=showAll&pageNo=<%=pageNum-1%>&limit=<%=limit%>">&lt;&lt;</a><%}%>	
	&nbsp;&nbsp;&nbsp;Page: <%=pageNum%>/<%=totalPages%>	
	<%if(pageNum==totalPages){%>&nbsp;&nbsp;&nbsp;&gt;&gt;&nbsp;&nbsp;&nbsp;&gt;|<%} else { %>&nbsp;&nbsp;&nbsp;<a href="complaintCntrl?page=complaintDashboard&opr=showAll&pageNo=<%=pageNo+1%>&limit=<%=limit%>">&gt;&gt;</a>&nbsp;&nbsp;&nbsp;<a href="complaintCntrl?page=complaintDashboard&opr=showAll&pageNo=<%=totalPages%>&limit=<%=limit%>">&gt;|</a><%}%>	
	</font>	
	</div>    	
	<%} %>	
...
<% while(it1.hasNext()){
	Complaint complaint1 = (Complaint)it1.next();
	%>
<tr>
<td><%=count++%></td>
<td><%=complaint1.getId()%></td>
<td><%=complaint1.getComplaintTypeValue()%></td>
<td><%=complaint1.getDetail()%></td>
<td><%=DateService.getDTSDDMMYYYY(complaint1.getComplaintDt()).equals("11-11-1111")?"":DateService.getDTSDDMMYYYY(complaint.getComplaintDt())%></td>
<td><%=DateService.getDTSDDMMYYYY(complaint1.getUpdateBy()).equals("11-11-1111")?"":DateService.getDTSDDMMYYYY(complaint.getUpdateBy())%></td>
<td><%=DateService.getDTSDDMMYYYY(complaint1.getUpdateTime()).equals("11-11-1111")?"":DateService.getDTSDDMMYYYY(complaint.getUpdateTime())%></td>
<td><%=complaint1.getComplaintStatusValue()%></td>
<td><%=DateService.getDTSDDMMYYYY(complaint1.getUpdatedt()).equals("11-11-1111")?"":DateService.getDTSDDMMYYYY(complaint.getUpdatedt())%></td>
<td><a href="complaintCntrl?page=saveShowNextComplaint&opr=edit&id=<%=complaint1.getId()%>&pageNo=<%=pageNo%>&limit=<%=limit%>"><button type="button" class="btn">Edit</button></a></td>
<td><a href="complaintCntrl?page=saveShowNextComplaint&opr=delete&id=<%=complaint1.getId()%>&pageNo=<%=pageNo%>&limit=<%=limit%>"><button type="button" class="btn1">Delete</button></a></td>
</tr>
	<%} %>
</tbody>
	</table>    </div>
